...
Bootstrap 初始化、收尾归档、迭代关闭检查不是角色，而是非角色机制。触发时机、执行者和结束条件见 `docs/baseline/mechanisms.md`。流程审计归 WM（工作流管理者）执行；当前角色只建议审计或做当前范围自检。关键结果必须由用户确认。
...
新增工作流角色必须通过 `role-wm.md` 的流程，并更新角色矩阵、迭代模板、日志和纠错文件。General 是入口默认角色，不适用工作流角色新增流程。
