...
`niuma-cheng-workboard` 是牛马程生态的跨项目 Agent 工作看板，用于只读聚合多个项目的团队工作流状态、接入诊断、跨项目需求、阻塞关系和协作进展。

## 技术栈

- 运行时：Node.js
- 第一版前端：静态 HTML / CSS / JavaScript
- 第一版后端：本地 Node HTTP 服务
- 第一版存储：无数据库，直接读取本地 Markdown 和配置文件

## 启动方式

待 Developer 实现。第一版目标为本地启动命令，例如：
