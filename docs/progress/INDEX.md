...
| P2 | **生态根「框架真源(agent-workflow)」卡片深度详情**（原「菜单 5→3」待办 ④ 子项的**剩余部分**）：菜单 5→3 主体（US-6 `13d31be`）+ 生态根卡片钻取**入口**（`a1685d8`，前端 App.tsx/EcosystemView）**已完成**；**唯剩** agent-workflow 卡片钻进去的详情仍薄——`workflow-source.js` 现仅浅层探测目录、拼一句 `kindSummary`，要展示 baseline/版本/演进需**后端扩展解析**。**不阻塞部署**。 | PM | Owner 口述（2026-07-06） | 待处理（剩后端深度详情，待 PM 定范围） |
