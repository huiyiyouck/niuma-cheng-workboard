...
| 优先级 | 待办 | 归属角色 | 来源 | 状态 |
|--------|------|----------|------|------|
| P2 | **生态根只读卡片钻取**（原「菜单 5→3」待办第 ④ 子项；**菜单 5→3 主体已纳入 v0.3 US-6，本点 v0.3 未覆盖故保留**）：项目会话视图里「公告板(coordination) / 框架真源(agent-workflow)」只读卡片点不进去，Owner 想看其「当前表现」。现状：公告板详情**已存在于需求池视图**（snapshot.crossProject：需求池/BCR/谁等谁/沟通），只缺卡片→详情跳转入口；agent-workflow 详情数据薄（snapshot 仅 kindSummary 一句），要展示 baseline/版本/演进需**后端 `workflow-source.js` 扩展解析**。给两张只读卡片加点击钻取（公告板→需求池视图；框架真源→框架状态详情+补后端）。 | PM | Owner 口述（2026-07-06） | 待研究 |
